...
print(bcolors.OKCYAN + "Deploying secrets to Kubernetes..." + bcolors.ENDC)
subprocess.run(['kubectl apply -k . -n %s' % sys.argv[1]], shell=True)

if DEBUG:
  print(bcolors.WARNING + "Debug Enabled: Skipping resource cleanup..." + bcolors.ENDC)
else:
  shutil.rmtree('.secrets')
  os.remove('kustomization.yaml')
